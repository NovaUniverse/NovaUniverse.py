...
class NovaCDN(NovaAPI):
    ...
    def is_cdn_endpoint(self) -> bool:
        ...
        if not self.silence_endpoint_warning:
            if "novauniverse.net/api" in self.endpoint:
                self.__logger.warn("You are using an API endpoint in the 'NovaCDN' class. Please only use api endpoints in the 'NovaAPI' class or else you'll run into many bugs.")
                return False
            else:
                return True
        return None
    ...
    def get(self) -> requests.Response:
        ...
        self.__logger.info(f"Sending cdn get request to '{self.endpoint}'...")
        response_file = self.__http_session.get(self.endpoint)

        if response_file.status_code == 200:
            self.__logger.info(f"CDN get request of '{self.endpoint}' was successful!")
